...
raw_db_url = os.getenv("DATABASE_URL", "sqlite:///./fallback.db")
...
# Models
# ---------------------------
class User(MentorBase):
    __tablename__ = "mc_users"
    id = Column(Integer, primary_key=True)
    name = Column(String(120), nullable=False)
    role = Column(String(20), default="user")  

class Assessment(MentorBase):
    __tablename__ = "mc_assessments"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey("mc_users.id"), nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)

class AssessmentResult(MentorBase):
    __tablename__ = "mc_assessment_results"
    id = Column(Integer, primary_key=True)
    assessment_id = Column(Integer, ForeignKey("mc_assessments.id"), nullable=False)
    skill_name = Column(String(80), nullable=False)
    score = Column(Integer, nullable=False) 
        

class MentorshipRequest(MentorBase):
    __tablename__ = "mc_mentorship_requests"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey("mc_users.id"), nullable=False)
...
class ChatMessageLog(MentorBase):
    __tablename__ = "mc_chat_messages"
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey("mc_users.id"), nullable=False)
    mentor_id = Column(String(120), nullable=False) # MongoDB Str ID
...
def startup():
    MentorBase.metadata.create_all(engine)
    db = SessionLocal()
    try:
        # SQLite Users
        ...
        if mentors_collection is not None:
            if mentors_collection.count_documents({}) == 0:
                print("Seeding Fake Mentors into MongoDB...")
                mentors_collection.insert_many([
                    {
                        "name": "Kavindu",
                        "contact": "kavindu@pathfinder.com",
                        "is_real": False,
                        "bio": "Senior PathFinder Mentor",
                        "active": True,
                        "expertise": {"Python": 9, "React": 8},
                        "sector": "Technology"
                    }
                ])
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Save to a temporary file locally with the correct filename to preserve the extension during Cloudinary upload
...
@router.post("/assessments", response_model=AssessmentCreateOut)
...
@router.get("/assessments/{assessment_id}/suggestions")
...
@router.get("/mentorship/my-requests")
...
@router.get("/mentorship/my-request")
...
@router.get("/mentor/requests/{mentor_id}")
...
@router.get("/mentor/students/{mentor_id}")
...
@router.get("/chat/history/{mentor_id}")
...
@router.delete("/chat/history/{mentor_id}")
...
def upload_chat_file(file: UploadFile = File(...)):
    """Securely uploads a file (PDF/Image) to Cloudinary to be attached in the chat."""
    MAX_FILE_SIZE = 8 * 1024 * 1024 # 8MB
    file.file.seek(0, 2)
    file_size = file.file.tell()
    file.file.seek(0)
...
@router.websocket("/ws/chat/{mentor_id}/{user_role}")
