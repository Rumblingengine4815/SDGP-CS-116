...
export default function DataSourcesPage() {
  return (
    <main className="min-h-screen bg-transparent text-foreground font-sans selection:bg-violet-100 selection:text-violet-900">
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-24">
        ...
        >
          <h1 className="text-4xl md:text-6xl font-bold text-foreground tracking-tight mb-8">
            The data that power{" "}
            <span className="text-blue-600 font-bold">PathFinder+</span>
          </h1>
          <p className="text-default-400 text-default-500 text-lg max-w-2xl mx-auto leading-relaxed font-medium">
            We aggregate thousands of data points from local and global sources
            to build your career trajectory with clinical precision.
          </p>
        </motion.div>
          ...
          {sources.map((section) => (
            <div key={section.category}>
              <div className="flex items-center gap-6 mb-12">
                <h2 className="text-[10px] font-black text-default-400 text-default-500 uppercase tracking-[0.5em] whitespace-nowrap">
                  {section.category}
                </h2>
                <div className="h-[1px] flex-1 bg-content4"></div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                {section.items.map((item) => (
                  <motion.div
                    ...
                    className="relative p-10 bg-content1 border border-divider rounded-3xl shadow-sm hover:shadow-xl hover:shadow-violet-200/40 transition-all duration-300 group/card"
                  >
                    <div className="flex flex-col sm:flex-row items-start gap-10">
                      <div className="flex-shrink-0 w-20 h-20 rounded-2xl bg-content1 border border-divider flex items-center justify-center p-3 shadow-sm group-hover/card:border-violet-100 transition-colors">
                        <Image
                          src={item.logo}
                          alt={item.name}
                          width={64}
                      ...
                      <div className="flex-1">
                        <h3 className="text-xl font-bold text-foreground mb-4 group-hover/card:text-violet-600 transition-colors">
                          {item.name}
                        </h3>
                        <p className="text-default-400 text-default-500 leading-relaxed text-sm">
                          {item.desc}
                        </p>
                      </div>
                    </div>
