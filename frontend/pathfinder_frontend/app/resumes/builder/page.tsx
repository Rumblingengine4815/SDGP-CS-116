...
export default function StandaloneResumeBuilder() {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      // Temporarily remove max-height restrictions for accurate canvas capture
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col items-center w-full">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-7xl px-8">
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4 print:hidden">
          ...
          <Button color="primary" variant="shadow" onPress={handlePrint} size="lg" className="w-full md:w-auto" isLoading={isGenerating}>
            {isGenerating ? "Generating File..." : "Download PDF"}
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 print:block print:w-full">
          ...
          <div id="printable-resume" className="rounded-lg shadow-2xl p-10 w-full max-w-[210mm] h-[297mm] mx-auto overflow-hidden print:p-10 print:shadow-none print:m-0 break-inside-avoid" style={{ backgroundColor: '#ffffff', color: '#000000' }}>
            ...
            <div className="mb-6">
              <h2 className="text-lg font-bold uppercase tracking-wider pb-1 mb-3" style={{ borderBottom: '2px solid #000000' }}>Professional Summary</h2>
              <p className="text-md leading-relaxed">{resumeData.summary}</p>
            </div>

            <div className="mb-6">
              <h2 className="text-lg font-bold uppercase tracking-wider pb-1 mb-3" style={{ borderBottom: '2px solid #000000' }}>Core Skills</h2>
              <p className="text-md leading-relaxed font-semibold">{resumeData.skills}</p>
            </div>

            <div className="mb-6">
              <h2 className="text-lg font-bold uppercase tracking-wider pb-1 mb-3" style={{ borderBottom: '2px solid #000000' }}>Professional Experience</h2>
            ...
            <div className="mb-6">
              <h2 className="text-lg font-bold uppercase tracking-wider pb-1 mb-3" style={{ borderBottom: '2px solid #000000' }}>Education</h2>
              <div className="whitespace-pre-wrap text-md leading-relaxed">
            ...
            {customSections.map((section, idx) => (
              <div key={idx} className="mb-6">
                <h2 className="text-lg font-bold uppercase tracking-wider pb-1 mb-3" style={{ borderBottom: '2px solid #000000' }}>{section.title || "Custom Section"}</h2>
                <div className="whitespace-pre-wrap text-md leading-relaxed">
                  {section.content}
                </div>
              </div>
        ...
        <style dangerouslySetInnerHTML={{__html: `
          @media print {
            html, body { background: white; margin: 0 !important; padding: 0 !important; overflow: hidden !important; height: 100% !important; }
            ...
            #printable-resume { position: absolute; left: 0; top: 0; width: 210mm; height: 297mm; max-height: 297mm; margin: 0; padding: 15mm; box-sizing: border-box; overflow: hidden; page-break-after: avoid; }
